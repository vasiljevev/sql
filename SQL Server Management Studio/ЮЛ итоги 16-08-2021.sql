select
    ...
    ,T.[Количество ЭЭ учтенной счетчиком] as [Количество ЭЭ учтенной счетчиком]
    ,T.[Коэф. учета]
    --,T.[Метод расчета ( по мощности, по ПУ, по нормативу, все какие используются)]
	...
	ABS(fpd.N_MR_Value_Prev-T.[Показания прибора учета предыдущего периода]) as [разница НП],
	ABS(fpd.N_MR_Value-T.[Показания прибора учета текущего периода]) as [разница КП],
	ABS(fpd.N_MR_Value_Prev-T.[Показания прибора учета предыдущего периода])+ABS(fpd.N_MR_Value-T.[Показания прибора учета текущего периода]) as [разница показ]
...
--ABS((T.[Полезный отпуск]- T.[объем корр]-FPD1.N_Quantity)*100/T.[Полезный отпуск]- T.[объем корр]) end as [diff полез%]
    ...
    FULL OUTER JOIN OMNIUS.TMP.TMP_PAYSHEETS_202107 T
        on t.F_Registr_Pts = erp.LINK
    where 1=1
    --and SS.N_Code is not NULL
	--and FPD.N_Quantity is not NULL
	--and t.link is  NULL
	--and t.F_Registr_Pts is  NULL
	and FPD.N_Quantity is not NULL
	and t.F_Registr_Pts is not NULL
	and abs(T.[Полезный отпуск]- T.[объем корр]-FPD1.N_Quantity)>10000
	and abs(T.[Полезный отпуск]- T.[объем корр]-FPD1.N_Quantity)<=100000
	--and ABS(fpd.N_MR_Value_Prev-T.[Показания прибора учета предыдущего периода])+ABS(fpd.N_MR_Value-T.[Показания прибора учета текущего периода])=0
